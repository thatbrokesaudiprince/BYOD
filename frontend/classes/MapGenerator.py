...
class MapGenerator:
    ...
    def __init__(self, tiles: str, attr: str, zoom_start: int = 6):
        ...
        self.GDELT_BATTLE_layer = folium.FeatureGroup(
            name="GDELT BATTLES", show=True, control=True
        )
        self.GDELT_PROTEST_layer = folium.FeatureGroup(
            name="GDELT PROTESTS", show=True, control=True
        )
        self.GDELT_RIOT_layer = folium.FeatureGroup(
            name="GDELT RIOTS", show=True, control=True
        )
        self.GDELT_EXPLOSION_layer = folium.FeatureGroup(
            name="GDELT EXPLOSION/REMOTE VIOLENCE", show=True, control=True
        )
        self.GDELT_VIOLENCE_layer = folium.FeatureGroup(
            name="GDELT VIOLENCE AGAINST CIVILLIANS", show=True, control=True
        )
        self.GDELT_STRATEGIC_layer = folium.FeatureGroup(
            name="GDELT STRATEGIC DEVELOPMENTS", show=True, control=True
        )
    ...
    def _create_marker_(
        ...
        customicon = folium.CustomIcon(
            icon_image=icon, icon_size=(19, 19), icon_anchor=(10, 10)
        )
    ...
    def _reinitialize_map_(
        ...
        if acled_data is not None and not acled_data.empty:
            ...
            # Add layers to the map here before adding the LayerControl

        if gdelt_data:
            # Handle GDELT data if necessary
            pass
        ...
        layers = [
            ...
            self.GDELT_BATTLE_layer,
            self.GDELT_EXPLOSION_layer,
            self.GDELT_PROTEST_layer,
            self.GDELT_RIOT_layer,
            self.GDELT_VIOLENCE_layer,
            self.GDELT_STRATEGIC_layer,
            self.CELLEX_layer,
        ]
        for layer in layers:
